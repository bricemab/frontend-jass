<template>
  <img v-if="!isLoaded" src="https://www.pngmart.com/files/21/Instagram-Logo-PNG-HD.png" style="display: none"
       @load="isLoadedImg"/>
  <div v-if="isLoaded" class="contact-container">
    ...
    <div class="contact-form">
      <div class="input-field is-label max-width">
        <label>{{ $t('dashboard.ideas.form.object') }} * <span
          style="font-size: 10px;">({{ object.length }} / 255)</span></label>
        <input type="text" class="input is-light max-width" v-model="object" v-on:input="validObjectLenght"
               :placeholder="$t('dashboard.ideas.form.object')">
      </div>
      <div class="input-field max-width is-label">
        <label>{{ $t('dashboard.ideas.form.content') }} *</label>
        <textarea :placeholder="$t('dashboard.ideas.form.content')" v-model="content"
                  class="input is-light max-width"></textarea>
      </div>
      <div class="send">
    ...
    <div class="contact-wrapper">
      <a class="contact" href="mailto:contact@mail.e-jass.ch">
...
@media screen and (max-width: 600px) {
  ...
  .dashboard-wrapper:has(.contact-container) {
    //.contact-container {
    //  width: 700px;
    //}
    h1 {
